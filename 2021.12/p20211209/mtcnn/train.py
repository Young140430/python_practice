...
class Trainer:
    ...
    def train(self):
        ...
        dataloader = DataLoader(faceDataset, batch_size=512, shuffle=True, num_workers=0,drop_last=False) # 数据加载器
        ...
        while True:
            for i, (img_data_, category_, offset_) in enumerate(dataloader): # 样本，置信度，偏移量
                ...
                if loss.cpu().data.numpy()<max_loss:
                    max_loss=loss.cpu().data.numpy()
                    # 保存
