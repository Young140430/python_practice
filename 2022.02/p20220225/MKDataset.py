...
class MKDataset(Dataset):
    def __init__(self, path):
        self.path = path
        self.name = os.listdir(os.path.join(path, 'DUTS-TR-Mask'))
        # self.name = path
    ...
    def __getitem__(self, index):
        ...
        namejpg = name[:-3] + 'jpg'
        img1_path = os.path.join(self.path, 'DUTS-TR-Image')
        img0_path = os.path.join(self.path, 'DUTS-TR-Mask')
...
if __name__ == '__main__':
    i = 1
    dataset = MKDataset(r"E:\baiduwangpan\BaiduNetdiskDownload\DUTS\DUTS-TR\DUTS-TR")
    for a, b in dataset:
        print(i)
        print(a.shape)
        print(b.shape)
        save_image(a,"E:/baiduwangpan/BaiduNetdiskDownload/DUTS/DUTS-TR/DUTS-TR/img/{0}.jpg".format(i),nrow=1)
        save_image(b,"E:/baiduwangpan/BaiduNetdiskDownload/DUTS/DUTS-TR/DUTS-TR/img/{0}.png".format(i),nrow=1)
        i+=1
