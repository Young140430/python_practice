...
if __name__ == '__main__':
    summaryWriter=SummaryWriter("logs_voc2007")
    myDataset = dataset2_1.MyDataset()
    train_loader = DataLoader(myDataset, batch_size=6, shuffle=True)
    ...
    while True:
        for target_13, target_26, target_52, img_data in train_loader:
            ...
            if loss.item() < max_loss:
                max_loss = loss.item()
                # 保存
                torch.save(net.state_dict(), r"E:\yolov3_param\voc_best2_1.pt")
                print("save voc_best2_1.pt success")
            print("最小的loss===>",max_loss)
            if epoch % 10 == 0 and epoch > 0:
                torch.save(net.state_dict(), r"E:\yolov3_param\voc_last2_1.pt")
                print("save voc_last2_1.pt success")
            epoch += 1
