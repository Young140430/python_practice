...
LABEL_FILE_PATH = "label2-1.txt"
IMG_BASE_DIR = r"F:\XunLeiDownload\VOCtrainval_06-Nov-2007\VOCdevkit\VOC2007\JPEGImages2"
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        index = 0
        for feature_size, anchors in cfg2_1.ANCHORS_GROUP.items():
            labels[feature_size] = np.zeros(shape=(feature_size, feature_size, 3, 5 + cfg2_1.CLASS_NUM))



            for box in boxes:
                cls, cx, cy, w, h = box
                cx_offset, cx_index = math.modf(cx * feature_size / cfg2_1.IMG_WIDTH)
                cy_offset, cy_index = math.modf(cy * feature_size / cfg2_1.IMG_WIDTH)

                for i, anchor in enumerate(anchors):

                    anchor_area = cfg2_1.ANCHORS_GROUP_AREA[feature_size][i]
                    p_w, p_h = w / anchor[0], h / anchor[1]
                    p_area = w * h
                    iou = min(p_area, anchor_area) / max(p_area, anchor_area)

                    index+=1
                    labels[feature_size][int(cy_index), int(cx_index), i] = np.array([iou, cx_offset, cy_offset, np.log(p_w), np.log(p_h), *one_hot(cfg2_1.CLASS_NUM, int(cls))])
