...
if __name__ == '__main__':
    summaryWriter=SummaryWriter("logs_pool_car")
    myDataset = dataset3.MyDataset()
    ...
    while True:
        for target_13, target_26, target_52, img_data in train_loader:
            ...
            if loss.item() < max_loss:
                max_loss = loss.item()
                # 保存
                torch.save(net.state_dict(), r"E:\yolov3_param\pool_car_best.pt")
                print("save pool_car_best.pt success")
            print("最小的loss===>",max_loss)
            if epoch % 10 == 0 and epoch > 0:
                torch.save(net.state_dict(), r"E:\yolov3_param\pool_car_last.pt")
                print("save pool_car_last.pt success")
            epoch += 1
