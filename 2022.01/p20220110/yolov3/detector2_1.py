...
def Net_out(image_data_):
    net_ = MobileNet()
    # print(net_)
    net_.load_state_dict(torch.load(r"E:\yolov3_param\voc_best2_1.pt"))
    net_.eval()
...
if __name__ == '__main__':
    ...
    cls_names = ['person','bird','cat','cow','dog','horse','sheep','aeroplane','bicycle','boat','bus','car','motorbike',
           'train','bottle','chair','diningtable','pottedplant','sofa','tvmonitor']

    path = r"F:\XunLeiDownload\VOCtest_06-Nov-2007\VOCdevkit\VOC2007\JPEGImages"
    for img_file in glob.glob("{}/*.jpg".format(path)):
        ...
        boxes_13 = get_boxes(idxs_13, vecs_13, 32, resize_h, cfg2_1.ANCHORS_GROUP[13])

        idxs_26, vecs_26 = get_offsets(output_26, 0.1)
        vecs_26[..., 5:] = torch.softmax(vecs_26[..., 5:], dim=1)
        boxes_26 = get_boxes(idxs_26, vecs_26, 16, resize_h, cfg2_1.ANCHORS_GROUP[26])

        idxs_52, vecs_52 = get_offsets(output_52, 0.1)
        vecs_52[..., 5:] = torch.softmax(vecs_52[..., 5:], dim=1)
        boxes_52 = get_boxes(idxs_52, vecs_52, 8, resize_h, cfg2_1.ANCHORS_GROUP[52])
        ...
        for box_ in box:
            ...
            cv2.putText(image, cls_names[cls_index], (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 0, 255))
