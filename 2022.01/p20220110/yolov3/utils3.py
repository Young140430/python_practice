...
def iou(box, boxes, isMin = False): #1st框，一堆框，inMin(IOU有两种：一个除以最小值，一个除以并集)
    ...
    #找交集：
    xx1 = torch.tensor(maximum(box[0], boxes[:, 0])).detach().numpy() #横坐标，左上角最大值
    yy1 = torch.tensor(maximum(box[1], boxes[:, 1])).detach().numpy() #纵坐标，左上角最大值
    xx2 = torch.tensor(minimum(box[2], boxes[:, 2])).detach().numpy() #横坐标，右下角最小值
    yy2 = torch.tensor(minimum(box[3], boxes[:, 3])).detach().numpy() #纵坐标，右小角最小值

    # 判断是否有交集
    w = torch.tensor(maximum(0, xx2 - xx1)).detach().numpy()
    h = torch.tensor(maximum(0, yy2 - yy1)).detach().numpy()

    #交集的面积
    inter = w * h  #对应位置元素相乘
    if isMin: #若果为False
        ovr = torch.true_divide(inter, np.minimum(box_area, area)) #最小面积的IOU：O网络用
    else:
        ovr = torch.true_divide(inter, (box_area + area - inter))  #并集的IOU：P和R网络用；交集/并集
...
def nms(boxes, thresh, isMin = False):
    ...
    while _boxes.shape[0] > 1: #shape[0]等价于shape(0),代表0轴上框的个数（维数）
        ...
        index = torch.where(iou(a_box, b_boxes,isMin) < thresh) #将阈值小于0.3的建议框保留下来，返回保留框的索引
